...
from sqlshare_web.views import (dataset_detail, dataset_list,
                                ...
                                upload_parser, user_search, upload_parser_init)
...
urlpatterns = [
    ...
    url(r'^upload/finalize_process/(?P<file_id>.*?)/(?P<filename>.*)',
        finalize_process,
        name='upload_finalize_process'),


    url(r'^upload/parser/(?P<file_id>.*?)/(?P<filename>.*)', upload_parser,
        name='upload_parser'),

    url(r'^upload/parser/(?P<filename>.*)', upload_parser_init,
        name='upload_parser_init'),
